...
echo
warn "This script will permanently delete comments from ALL WordPress sites found under '${SEARCH_PATH}'."
...
info "Starting search for WordPress installations in '${SEARCH_PATH}'..."
...
while IFS= read -r config_file; do
  ...
  echo -e "\n${C_INFO}===============================================================${C_RESET}"
  info "Processing site at: ${wp_path}"
  ...
  (
    ...
    if [[ $? -ne 0 ]]; then
      error "Failed to query comments. Skipping this site."
...
done < <(find "$SEARCH_PATH" -name "wp-config.php" -type f 2>/dev/null)

echo -e "\n${C_SUCCESS}======================= SUMMARY =======================${C_RESET}"

success "Script finished."
success "Processed a total of ${SITES_PROCESSED} potential WordPress sites."
success "Permanently deleted a grand total of ${TOTAL_DELETED} comments."
